...
def GAPS_IO_GetDim(filename):
	...
	#print tp
	numdim=readi64(fp)
	allblockver=list(range(numdim+1))
	alldimver=list(range(numdim))
	if version==0:
		alldimver=list(range(numdim+1))
	numperstep=1
...
def cdr(lst):
	lst.remove(lst[0])
	return lst
def GAPS_IO_Save_Position(filename,locarray,data):
	...
	if type(data)==type(0.5):
		array([data]).tofile(fp)
	else:
		data.tofile(fp)

def GAPS_IO_Load_Position(filename,locarray,num=0):
	dimarr=GAPS_IO_GetDim(filename)
	locarray.reverse()
	#dimarr.reverse()
	flip(dimarr)
	...
	if num!=0:
		nnum=num
	ret=numpy.fromfile(fp,dtype=get_type_from_int(tp),count=nnum)
	if num==0:
		return ret[0]
	else:
		return ret
...
def GAPS_IO_Load_Positions_3D(filename,lastnum):
	...
	return frt
def GAPS_IO_Save_Positions_3D(filename,frt,lastnum):
	dimarr=GAPS_IO_GetDim(filename)
	dimarr.reverse()
	fp=open(filename,"rb+")
	version=readi64(fp)
...
def GAPS_IO_Load(filename):
	...
	fp.seek(datahead)
	#ret=zeros(alldimver)
	#print(get_type_from_int(tp),alldatalen)
...
def GAPS_IO_Save(filename,data,dtype=7,version=0):
	...
	allblockver=array(list(range(numdim+1)))
	alldimver=array(list(range(numdim)))
	if version==0:
